...
class InstagridViewController: UIViewController {
    ...
    //centralView where assembly photos picked by the user
    @IBOutlet weak var centralView: UIView!
    
    //constrainst to manage the centralView animation
    @IBOutlet weak var centralXConstraint: NSLayoutConstraint!
    @IBOutlet weak var centralYConstraint: NSLayoutConstraint!
    
    // var used to factorise the methood UIImagePicker
    ...
    private func animationYCentral(constraint: CGFloat) {
        UIView.animate(withDuration: 0.5) {
            self.centralYConstraint.constant = constraint
                self.view.layoutIfNeeded()
        }
    }
